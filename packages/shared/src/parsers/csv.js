 ...
 * - 'Watson' -> 'jjols'
 */
export function normalizePetName(rawName) {
    const trimmed = (rawName || '').trim();
    const lower = trimmed.toLowerCase();
 ...
 */
export function parseDateTimeToUtcIso(logDate, logTime) {
 ...
 */
export function parseReportCsv(csvText) {
 ...
 */
export function summarizeReportCsvImport(items) {
    if (items.length === 0) {
        return {
            sourceType: 'csv',
            totalCount: 0,
 ...
 */
export function convertReportCsvToPetEvents(items, householdId, petId) {
    return items.map((item) => {
        ...
        if (item.weightKg !== undefined)
            metadata.weightKg = item.weightKg;
        if (item.weightLbs !== undefined)
            metadata.weightLbs = item.weightLbs;
        if (item.temperatureC !== undefined)
            metadata.temperatureC = item.temperatureC;
        if (item.temperatureF !== undefined)
            metadata.temperatureF = item.temperatureF;
        if (item.medicineType)
            metadata.medication = item.medicineType;
        if (item.stoolQuality)
            metadata.stoolQuality = item.stoolQuality;
        if (item.vaccineName)
            metadata.vaccineName = item.vaccineName;
        if (item.vaccineExpiration)
            metadata.vaccineExpiration = item.vaccineExpiration;
        if (item.bloodGlucoseNumber !== undefined)
            metadata.bloodGlucoseNumber = item.bloodGlucoseNumber;
        if (item.bloodGlucoseUnit)
            metadata.bloodGlucoseUnit = item.bloodGlucoseUnit;
        if (item.duration && item.duration !== '0')
            metadata.duration = item.duration;
        if (item.quantityNumber !== undefined)
            metadata.quantityNumber = item.quantityNumber;
        if (item.quantityUnit)
            metadata.quantityUnit = item.quantityUnit;
        return {
            householdId,
            petId,
            eventType,
