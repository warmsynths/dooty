 ...
 */
export function parseAnyImportFile(content: string, filename?: string): ParsedImportResult {
  ...
  if (isJson) {
    try {
      ...
      const universalSummary: UniversalImportSummary = {
        sourceType: 'json',
  ...
  try {
    ...
    throw new Error(
      `Failed to parse import file. Supported formats are CSV (e.g. report.csv) and DogNotes JSON. Detail: ${err.message}`
    );
  }
}
 ...
 */
export function convertAnyImportToPetEvents(
