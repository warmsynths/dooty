...
function circDiffMinutes(m1, m2) {
...
export function predictNextPoop(events, petId, now = new Date()) {
    ...
    const routineMinutes = detectedPeaks.length > 0
    ...
    for (const poop of petPoops) {
        const priorFoods = petFoods.filter((f) => f.date.getTime() < poop.date.getTime() && poop.date.getTime() - f.date.getTime() <= 3.5 * 3600000);
    ...
    const learnedFoodLagMins = foodLagsMins.length >= 2
    ...
    const walkLagsMins = [];
    for (const poop of petPoops) {
        const priorWalks = petWalks.filter((w) => fDateWithin(w.date, poop.date, 0, 90));
    ...
    function fDateWithin(d1, d2, minM, maxM) {
        const diff = (d2.getTime() - d1.getTime()) / 60000;
        return diff >= minM && diff <= maxM;
    }
    const learnedWalkLagMins = walkLagsMins.length >= 2
    ...
    if (walkCandidate && (walkCandidate.getTime() > nowMs - 45 * 60000)) {
        ...
        const overdueCandidate = todayCandidates.find((c) => c.getTime() <= nowMs && nowMs - c.getTime() <= 2.5 * 3600000 && c.getTime() >= minEarliestMs - 30 * 60000);
        // Future candidate today
        const futureTodayCandidate = todayCandidates.find((c) => c.getTime() > nowMs && c.getTime() >= minEarliestMs);
        // Overdue check based on inter-poop interval
        const isIntervalOverdue = (avgPoopsPerDay > 1.2 && hoursSinceLastPoop > typicalIntervalHours * 1.35 && now.getHours() >= 7 && now.getHours() <= 22) ||
...
export function calculatePetAnalytics(events, petId, now = new Date()) {
    ...
    for (const event of petEvents) {
        ...
        if (hourlyDistribution[hour]) {
            ...
            if (event.eventType === 'poop')
                hourlyDistribution[hour].poopCount++;
            if (event.eventType === 'pee')
                hourlyDistribution[hour].peeCount++;
        }
        // Counts by type
        ...
        // Last event
        if (!lastEventByType[event.eventType] ||
        ...
        if (event.eventType === 'poop')
            ...
            daily.medicine++;
        else if (event.eventType === 'vomit')
            daily.vomit++;
        else
            daily.other++;
        // Health watch
        if (d >= sevenDaysAgo) {
            if (event.eventType === 'vomit')
                vomitsLast7Days++;
    ...
    if (latestPoopTime) {
        daysWithoutPoop = Math.max(0, Math.floor((now.getTime() - latestPoopTime.getTime()) / (24 * 60 * 60 * 1000)));
    }
    // Sorted daily frequencies
    const dailyFrequencies = Array.from(dailyMap.values()).sort((a, b) => a.date.localeCompare(b.date));
