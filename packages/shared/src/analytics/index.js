export function predictNextPoop(events, petId, now = new Date()) {
    ...
    let predictedDate;
    let predictionType = 'routine_today';
    let isTomorrow = false;
    let isOverdue = false;
    // Future routine hours today
    const futureRoutineHoursToday = routineHours.filter((h) => {
        const candidateDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h, 0, 0, 0);
    ...
    // Check if overdue
    const isOverdueCheck = (avgPoopsPerDay > 1.2 && hoursSinceLastPoop > avgIntervalHours * 1.35 && currentHourFraction >= 7 && currentHourFraction <= 22) ||
        (avgPoopsPerDay <= 1.2 && poopsTodayCount === 0 && currentHourFraction >= 14 && hoursSinceLastPoop >= 20);
    if (isOverdueCheck) {
        isOverdue = true;
        predictionType = 'overdue';
        if (futureRoutineHoursToday.length > 0 &&
            futureRoutineHoursToday[0] - currentHourFraction <= 2.0) {
            predictedDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), futureRoutineHoursToday[0], 0, 0, 0);
        }
        else {
        ...
        predictedDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), targetHour, 0, 0, 0);
        predictionType = 'routine_today';
    }
    else if (poopsTodayCount < Math.ceil(avgPoopsPerDay) &&
        avgPoopsPerDay > 1.2 &&
        ...
        predictedDate = new Date(now.getFullYear(), now.getMonth(), now.getDate() + 1, firstRoutineHour, 0, 0, 0);
    }
