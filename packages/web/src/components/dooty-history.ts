...
import { PetEvent, EventType } from '@dooty/shared';

@customElement('dooty-history')
export class DootyHistory extends LitElement {
  private unsubscribe?: () => void;

  @state() private searchSheetOpen: boolean = false;
  ...
  private getTypeTag(type: string, isKo: boolean): string {
    switch (type) {
  ...
  private getTypeName(type: string, isKo: boolean): string {
    switch (type) {
  ...
  private getEventsForDay(year: number, month: number, day: number): PetEvent[] {
    return (appState.events || []).filter((e) => {
      ...
      if (appState.historyMemberFilter !== 'all') {
        ...
        if (who !== appState.historyMemberFilter) return false;
      }
      return true;
    });
  }
  ...
  render() {
    ...
    const memberList = Array.from(
      new Set(
        (appState.currentHousehold?.members || []).map((m) => m.displayName).concat(['Me'])
      )
    );
        ...
        <div class="calendar-card">
          ...
          <div class="cal-header">
            ...
            <div
              ...
              @click=${() => {
                if (!isCurrentMonth) appState.setHistoryMonthOffset(appState.historyMonthOffset + 1);
              }}
            >
              &#8250;
            </div>
          ...
          <div class="day-names-row">
            ${(isKo ? ['월', '화', '수', '목', '금', '토', '일'] : ['M', 'T', 'W', 'T', 'F', 'S', 'S']).map(
              (name) => html`<div class="day-name">${name}</div>`
            )}
          </div>
            ...
            ${Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return html`
                <div
                  ...
                  @click=${() => {
                    if (selectedDay === day) appState.setHistorySelectedDay(null);
                    else appState.setHistorySelectedDay(day);
                  }}
                >
      ...
      ${this.searchSheetOpen
        ? html`
            <div class="search-modal-backdrop" @click=${() => (this.searchSheetOpen = false)}>
              <div class="search-modal" @click=${(e: Event) => e.stopPropagation()}>
