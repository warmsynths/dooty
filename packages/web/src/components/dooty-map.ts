...
export class DootyMap extends LitElement {
  @state() private activeFilter: 'all' | 'poop' | 'pee' | 'walk' = 'all';
  @state() private isLocating: boolean = false;
  @state() private selectedEventId?: string;
  ...
  private getFilteredEvents(): PetEvent[] {
    const geo = this.getGeoEvents();
    if (this.activeFilter === 'all') return geo;
    return geo.filter((e) => e.eventType === this.activeFilter);
  }

  private initMap() {
    const mapContainer = this.renderRoot?.querySelector(
      '#leaflet-map'
    ) as HTMLElement;
    if (!mapContainer || this.map) return;

    const geoEvents = this.getGeoEvents();
    let initialCenter: [number, number] = [37.5665, 126.978];
    let initialZoom = 13;

    if (geoEvents.length > 0) {
      initialCenter = [
        geoEvents[0].latitude as number,
        geoEvents[0].longitude as number,
      ];
      initialZoom = 15;
    }
  ...
  private updateMapLayers(fitToAll: boolean = false) {
    if (
      !this.map ||
      !this.markersLayer ||
      !this.trailsLayer ||
    ...
    if (allGeoEvents.length >= 3) {
      const coordPoints: [number, number][] = allGeoEvents.map((e) => [
  ...
  private handleFitAll() {
    ...
    const bounds = L.latLngBounds(
      geo.map((e) => [e.latitude as number, e.longitude as number])
    );
    this.map.fitBounds(bounds, { padding: [40, 40], maxZoom: 17 });
  }

  private handleSpotClick(spot: PetEvent) {
