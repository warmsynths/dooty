...
export class DootyWalk extends LitElement {
  ...
  private liveMap?: L.Map;
  private livePolyline?: L.Polyline;
  private livePolylineShadow?: L.Polyline;
  private liveStartMarker?: L.Marker;
  private liveCurrentMarker?: L.Marker;

  private summaryMap?: L.Map;
  ...
  private initOrUpdateLiveMap() {
    const container = this.renderRoot?.querySelector('#live-leaflet-map') as HTMLElement;
    if (!container) return;
  ...
  private handleRecenterLive() {
    if (!this.liveMap || !appState.activeWalk) return;
    const walk = appState.activeWalk;
  ...
  private initOrUpdateSummaryMap() {
    const container = this.renderRoot?.querySelector('#summary-leaflet-map') as HTMLElement;
    if (!container) return;
    ...
    if (!this.summaryMap) {
      ...
      const startPt: [number, number] | undefined =
        summary.startLat !== undefined && summary.startLng !== undefined
          ? [summary.startLat, summary.startLng]
          : route.length > 0
          ? [route[0][0], route[0][1]]
      ...
      const endPt: [number, number] | undefined =
