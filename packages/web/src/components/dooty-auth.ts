...
export class DootyAuth extends LitElement {
  // Auth view mode: 'signin' | 'signup' | 'dogsetup' | 'join' | 'joindetails'
  @state() private view: 'signin' | 'signup' | 'dogsetup' | 'join' | 'joindetails' = 'signin';
  ...
  @state() private trackingPrefs: Record<string, boolean> = {
    poop: true,
    pee: true,
    vomit: true,
    meds: true,
  ...
  @state() private joinRole: string = 'Dan the walker';
  ...
  private setView(nextView: 'signin' | 'signup' | 'dogsetup' | 'join' | 'joindetails') {
    this.view = nextView;
    appState.setAuthView(nextView);
    this.errorMessage = '';
  }

  private calculateStrength(pwd: string): { label: string; width: string; color: string } {
    const t = appState.t.auth.signupStep1;
    if (!pwd || pwd.length < 6) return { label: t.weak, width: '25%', color: '#FF5A3C' };
  ...
  private handleGoToStep2(e?: Event) {
    if (e) e.preventDefault();
    this.errorMessage = '';
    const t = appState.t.auth.errors;
  ...
  private async handleFinishSetup(e?: Event) {
    if (e) e.preventDefault();
    this.errorMessage = '';
    const t = appState.t.auth.errors;
  ...
  private handleGoJoinDetails(e?: Event) {
    if (e) e.preventDefault();
    this.errorMessage = '';
    const t = appState.t.auth.errors;
  ...
  private async handleJoinSubmit(e?: Event) {
    if (e) e.preventDefault();
    this.errorMessage = '';
    const t = appState.t.auth.errors;
  ...
  render() {
    ...
    if (this.view === 'signin') {
      return html`
        <div class="view-signin">
          ...
          <form class="card" @submit=${(e: Event) => this.handleLogin(e)}>
            <div>
    ...
    if (this.view === 'signup') {
      ...
      return html`
        <div class="view-signup">
          ...
          <form class="card" @submit=${(e: Event) => this.handleGoToStep2(e)}>
            <div>
    ...
    if (this.view === 'dogsetup') {
      ...
      const sizeList: Array<'S' | 'M' | 'L' | 'XL'> = ['S', 'M', 'L', 'XL'];
      ...
      return html`
        <div class="view-dogsetup">
          ...
          <form class="card" @submit=${(e: Event) => this.handleFinishSetup(e)}>
            ...
            <div>
              ...
              <input
                ...
                @input=${(e: any) => (this.householdName = e.target.value)}
              />
    ...
    if (this.view === 'join') {
      ...
      return html`
        <div class="view-join">
          ...
          <form class="card" @submit=${(e: Event) => this.handleGoJoinDetails(e)}>
    ...
    if (this.view === 'joindetails') {
      ...
      return html`
        <div class="view-joindetails">
          ...
          <form class="card" @submit=${(e: Event) => this.handleJoinSubmit(e)}>
