...
export class DootySheet extends LitElement {
  ...
  @state() private lat?: number = undefined;
  @state() private lng?: number = undefined;
  @state() private isLocating: boolean = false;
  @state() private showLocationPicker: boolean = false;
  @state() private showMapPicker: boolean = false;
  @state() private showTimePicker: boolean = false;
  @state() private customTimestamp: string = '';
  @state() private walkPetIds: string[] = [];
  @state() private weatherText: string = '';
  @state() private isFetchingWeather: boolean = false;
  @state() private isSaving: boolean = false;

  // Walk Start/End GPS State
  @state() private startLat?: number = undefined;
  @state() private startLng?: number = undefined;
  @state() private startLocationName: string = '';
  @state() private isLocatingStart: boolean = false;
  @state() private endLat?: number = undefined;
  @state() private endLng?: number = undefined;
  @state() private endLocationName: string = '';
  @state() private isLocatingEnd: boolean = false;
  @state() private activeMapPickerTarget: 'single' | 'start' | 'end' = 'single';
      ...
      if (appState.loggerModalOpen) {
        if (!this.wasOpen) {
          ...
          if (appState.editingEvent) {
            ...
            if (meta.consistency) this.cons = meta.consistency;
            if (meta.size) this.size = meta.size;
            if (meta.mood) this.mood = meta.mood;
            if (meta.medication) this.selectedMed = meta.medication;
            if (meta.dosage) this.selectedMedDose = meta.dosage;
            if (meta.weightKg) this.weightKg = meta.weightKg;
            if (meta.walkDuration) this.walkMin = meta.walkDuration;
            if (meta.walkDistance) this.walkKm = meta.walkDistance;
            if (meta.visitReason) this.vetReason = meta.visitReason;
            if (meta.symptom) this.symptom = meta.symptom;
            if (meta.portion) this.portion = meta.portion;
  ...
  private moodOptions = MOOD_OPTIONS;
  ...
  private wmoCodeToDescription(code: number): string {
    ...
    if (entry) return isKo ? entry[1] : entry[0];
  ...
  private formatDisplayTime(isoString?: string): { main: string; sub: string } {
    const isKo = appState.currentLocale === 'ko';
  ...
  private handleCustomTimeInput(val: string) {
    if (!val) {
      this.customTimestamp = new Date().toISOString();
    } else {
  ...
  private setQuickOffsetMinutes(offsetMinutes: number) {
    const target = new Date(Date.now() - offsetMinutes * 60 * 1000);
    this.customTimestamp = target.toISOString();
    this.requestUpdate();
  }

  private setQuickOffsetDays(offsetDays: number) {
  ...
  private async handleSave() {
    ...
    if (type === 'poop') {
      ...
      if (this.startLat !== undefined && this.startLng !== undefined) {
        ...
        if (!this.locationName) this.locationName = this.startLocationName;
      }
  ...
  private async handleDelete() {
    if (!appState.editingEvent) return;
    const isKo = appState.currentLocale === 'ko';
  ...
  private openMapPickerFor(target: 'single' | 'start' | 'end') {
    this.activeMapPickerTarget = target;
    this.showMapPicker = true;
    this.requestUpdate();
  }

  private async fetchStartGPS() {
    if (typeof navigator === 'undefined' || !navigator.geolocation) return;
  ...
  private async tryReverseGeocodeForTarget(lat: number, lng: number, target: 'single' | 'start' | 'end') {
    try {
      const res = await fetch(
        `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=1`,
      ...
      if (res.ok) {
        ...
        if (name) {
          if (target === 'start') this.startLocationName = name;
          else if (target === 'end') this.endLocationName = name;
          else this.locationName = name;
          this.requestUpdate();
        }
      }
    } catch {
  ...
  private handleSpotSelected(e: CustomEvent<{ lat?: number; lng?: number; locationName: string }>) {
    const lat = e.detail.lat;
    const lng = e.detail.lng;
    const locName =
      e.detail.locationName ||
    ...
    return html`
      <div class="sheet-overlay">
                    ...
                    <div class="pill-row">
                      <div
                        ...
                        @click=${() => {
                          ...
                          if (this.showTimePicker) this.showLocationPicker = false;
                        }}
                      >
                          ...
                          <div class="time-picker-card">
                            ...
                            <div class="custom-loc-input-row">
                              <input
                                ...
                                @input=${(e: any) => this.handleCustomTimeInput(e.target.value)}
                              />
                            </div>
                          </div>
                        `
                          ...
                          <div style="display: flex; flex-direction: column; gap: 14px;">
                              ...
                              <div class="walk-custom-inputs-row">
                                <div class="walk-input-box">
                                  ...
                                  <input
                                    ...
                                    placeholder="30 min"
                                    .value=${this.walkMin}
                                    @input=${(e: any) => (this.walkMin = e.target.value)}
                                  />
                                </div>
                                <div class="walk-input-box">
                                  ...
                                  <input
                                    ...
                                    placeholder="2.3 km"
                                    .value=${this.walkKm}
                                    @input=${(e: any) => (this.walkKm = e.target.value)}
                                  />
                                </div>
                              </div>
                            </div>
                            ...
                            <div class="walk-gps-card">
                              ...
                              <input
                                ...
                                @input=${(e: any) => (this.startLocationName = e.target.value)}
                              />
                            ...
                            <div class="walk-gps-card">
                              ...
                              <input
                                ...
                                @input=${(e: any) => (this.endLocationName = e.target.value)}
                              />
                    ...
                    ${!showWalk && this.showLocationPicker
                      ? html`
                          <div class="location-picker-card">
                            ...
                            <div class="location-chips-row">
                              ${(isKo ? this.locationPresetsKo : this.locationPresets).map(
                                (preset) => html`
                                  <div
                            ...
                            <div class="custom-loc-input-row">
                              <input
                                ...
                                @input=${(e: any) => (this.locationName = e.target.value)}
                              />
                            </div>
                          </div>
                        `
        ...
        <dooty-map-picker
          ...
          @spot-selected=${(e: CustomEvent) => this.handleSpotSelected(e)}
