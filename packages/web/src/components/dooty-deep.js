...
let DootyDeep = class DootyDeep extends LitElement {
    constructor() {
        super(...arguments);
        this.distNames = ['Pellets', 'Lumpy', 'Cracked', 'Textbook', 'Soft blobs', 'Mushy', 'Liquid'];
        this.distCol = ['#E3D8BE', '#E3D8BE', '#FFE9A8', '#1FC99B', '#FFCE2E', '#FF9A3C', '#FF5A3C'];
    }
  ...
  `; }
    render() {
        ...
        const distP = counts.map((cnt) => totalPoopClassified > 0 ? Math.round((cnt / totalPoopClassified) * 100) : 0);
        const perfectCount = counts[3];
        const gutScore = totalPoopClassified > 0
            ? Math.round((perfectCount / totalPoopClassified) * 100)
            : totalCount > 0
                ? 100
                : 0;
        // Health alert events in last 14 days
        const flaggedEvents = events.filter((e) => (e.eventType === 'vomit' || (e.notes || '').toLowerCase().includes('loose') || (e.notes || '').toLowerCase().includes('diarrhea')) &&
...
DootyDeep = __decorate([
    customElement('dooty-deep')
], DootyDeep);
export { DootyDeep };
//# sourceMappingURL=dooty-deep.js.map