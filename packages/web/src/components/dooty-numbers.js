...
let DootyNumbers = class DootyNumbers extends LitElement {
  ...
  `; }
    ...
    render() {
        ...
        const quietStrEn = minQuietSum === 0
        ...
        const quietStrKo = minQuietSum === 0
        ...
        const recentPoops = events.filter((e) => e.eventType === 'poop' && new Date(e.timestamp) >= fourteenDaysAgo);
        const perfectCount = recentPoops.filter((e) => (e.notes || '').toLowerCase().includes('4') || (e.notes || '').toLowerCase().includes('textbook')).length;
        const gutScore = recentPoops.length > 0
        ...
        if (poopEventsList.length >= 2) {
            const sorted = [...poopEventsList].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
        ...
        const flagCount = events.filter((e) => e.eventType === 'vomit' && new Date(e.timestamp) >= sevenDaysAgo).length;
        ...
        const rawMorningDev = totalCount > 0 ? Math.round(((morningRate - avgEventsPerHour) / avgEventsPerHour) * 100) : 25;
        ...
        const rawAfternoonDev = totalCount > 0 ? Math.round(((afternoonRate - avgEventsPerHour) / avgEventsPerHour) * 100) : 10;
        // 3. Weekend vs Weekday
        const weekendEvents = heatmapCounts[5].reduce((a, b) => a + b, 0) + heatmapCounts[6].reduce((a, b) => a + b, 0);
        const weekdayEvents = totalCount - weekendEvents;
        const weekendRate = weekendEvents / 2;
        const weekdayRate = weekdayEvents / 5;
        const rawWeekendDev = weekdayRate > 0 && totalCount > 0
            ? Math.round(((weekendRate - weekdayRate) / weekdayRate) * 100)
            : totalCount > 0
                ? 0
                : 14;
        // 4. Night (21:00 - 5:59) - 9 hours
        const nightTotal = hourCounts[21] +
            hourCounts[22] +
            hourCounts[23] +
            hourCounts[0] +
            hourCounts[1] +
        ...
        const rawNightDev = totalCount > 0 ? Math.round(((nightRate - avgEventsPerHour) / avgEventsPerHour) * 100) : -65;
        const makeCorrItem = (label, dev, color) => {
        ...
        const rangeBadgeText = timeRange === '7d'
            ? isKo ? '7일' : '7 DAYS'
            : timeRange === '30d'
                ? isKo ? '30일' : '30 DAYS'
                : timeRange === '1y'
...
__decorate([
    state()
], DootyNumbers.prototype, "unsubscribe", void 0);
DootyNumbers = __decorate([
    customElement('dooty-numbers')
], DootyNumbers);
export { DootyNumbers };
//# sourceMappingURL=dooty-numbers.js.map