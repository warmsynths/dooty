...
class AppStateManager {
  ...
  treatmentsDrawerOpen: boolean = false;
  ...
  formatTreatmentLeft(due: number): string {
    ...
    if (due === 0) return isKo ? '오늘' : 'Today';
    if (due === 1) return isKo ? '내일' : 'Tomorrow';
  ...
  giveTreatment(id: string): { title: string; sub: string } {
    const isKo = this.currentLocale === 'ko';
  ...
  addTreatment(opts: {
    ...
    const newItem: TreatmentScheduleItem = {
      id,
  ...
  removeTreatment(id: string): { title: string; sub: string } {
    const isKo = this.currentLocale === 'ko';
    const item = this.treatments.find((t) => t.id === id);
    ...
    if (this.isAuthenticated) {
      ApiClient.deleteTreatmentSchedule(id).catch((e) =>
        console.warn('Could not sync treatment deletion to server:', e)
      );
    }
